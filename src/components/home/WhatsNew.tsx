import { Box, Button, Container, Heading, SimpleGrid, GridItem, VStack, Text, UnorderedList, ListItem, Image, Flex } from '@chakra-ui/react';
...
export const WhatsNew: React.FC = () => {
  return (
    <Box as="section" pb={{ base: '4rem', md: '8rem' }} bgGradient="linear(to-b, #52B4EC, #6983FF)">
      <Container maxW={'7xl'}>
        <Heading
          textAlign={'center'}
          fontSize={'5xl'}
        ...
        <SimpleGrid columns={{ base: 1, lg: 3 }} spacing={8} p={'8'} borderRadius={'20px'} bg={'rgba(255, 255, 255, 0.2)'}>
            ...
            <GridItem colSpan={{ base: 1, md: 2 }}>
                <Box>
                    ...
                    <Flex justifyContent={'space-between'} alignItems={'flex-start'}>
                        <Box>
                            <Text fontSize="20px" fontWeight="bold" color={'white'}>Version: {releaseNotes.version}</Text>
                            <Text fontSize="20px" fontWeight="400" color={'white'}>Release Date: {releaseNotes.releaseDate}</Text>
                        </Box>
                        <Button
                            as="a"
                            href="#"
                    ...
                    <VStack align="stretch" spacing={6} background={'white'} borderRadius={'20px'} mt={'6'} padding={'8'}>
                    {releaseNotes.sections.map((section) => (
                        ...
                        <UnorderedList spacing={2} fontSize={'16px'}>
                            {section.items.map((item, index) => (
                                <ListItem borderBottom={'1px solid'} borderColor={'rgba(157, 157, 157, 0.5)'} mb={'2'} pb={'2'} key={index}>{item}</ListItem>
                            ))}
                        </UnorderedList>
                        </Box>
                    ))}
                ...
                <Box>
                    ...
                    {latestBlogPosts.map((post, index) => (
                        ...
                        <Box p={6}>
                            ...
                            <Text mb={4} fontSize={"16px"}>{post.description}</Text>
                            <Box textAlign={'right'}>
                            <Button
                                as="a"
